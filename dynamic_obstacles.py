...
║  Pipeline: TCP → Декодирование → Кластеризация → Трекинг → КФ   ║
╠══════════════════════════════════════════════════════════════════╣
║  Управление:                                                      ║
║    Q         — выход                                              ║
...
def suppress_shadows(tracks: list) -> None:
    """
    Два правила подавления ложных динамических объектов:

    Правило 1 — Угловой конус (классическая тень лидара):
      Если трек находится в угловом секторе другого динамического трека
      И при этом дальше от лидара — это тень, подавляем.

    Правило 2 — Ближайший побеждает (кластеры в одной зоне):
      Среди динамических треков в радиусе SHADOW_ZONE_MM друг от друга
      оставляем только тот, что ближе к лидару. Остальные — артефакты,
      порождённые изменением фона рядом с реальным объектом.
    """
